...
class Luong_Helper_DPaging {
    ...
    public function doPaging($totalRecords = 0, $urlAction = "", $currentPages = 1, $itemPerPages = 0, $offset = 0, $navigation = array(), $class = "", $class2 = "") {
        ...
        $response = "";
        if ($totalPages > 1) {
            $response.='<div class="dataTables_paginate fg-buttonset ui-buttonset fg-buttonset-multi ui-buttonset-multi paging_full_numbers" id="jtable_paginate">';
            if ($totalPages > $offset) {
                if ($currentPages > $offset - $page_offset) {
                    $response.="<a class='$class' href='" . $urlAction . "?page=1'>" . $a[0] . "</a>&nbsp;...&nbsp;";
                }
            }
            if (($currentPages - 1) > 0) {
                $response.="<a class='$class' href='" . $urlAction . "?page=" . ($currentPages - 1) . "'>" . $a[1] . "</a>";
            }
            if ($totalPages > $offset) {
            ...
            for ($i = $idx_fst; $i <= $idx_lst; $i++) {
                if ($i == $currentPages) {
                    ...
                    $response.='<a tabindex="0" class="' . $class . '" href=' . $urlAction . '?page=' . $i . '>' . $i . '</a>';
                }
            }
            if (($currentPages + 1) <= $totalPages) {
                $response.="<a class='$class' href='" . $urlAction . "?page=" . ($currentPages + 1) . "'>" . $a[2] . "</a>";
            }
            if ($totalPages > $offset) {
                if ($currentPages <= $totalPages - $offset + $page_offset) {
                    $response.="&nbsp;...&nbsp;<a class='$class' href='" . $urlAction . "?page=$totalPages'>" . $a[3] . "</a>";
                }
            }
            $response.="</div>";
        }
